...
internal class GetProductsForShopQueryHandler : IQueryHandler<GetProductsForShopQuery, ProductShopResult>
{
    ...
    public async Task<ProductShopResult> Handle(GetProductsForShopQuery request, CancellationToken cancellationToken)
    {
        ...
        {
            var category = await _context.Categories.FirstOrDefaultAsync(f => f.Slug == @params.Slug, cancellationToken);

            if (category != null)
            {
                conditions += @$" and (A.CategoryId={category.Id} or A.SubCategoryId={category.Id}
                              or A.SecondarySubCategoryId={category.Id})";
                selectedCategory = category.MapCategory();
            }
        }
        ...
        switch (@params.ProductSearchOrderBy)
        {
            ...
            case ProductSearchOrderBy.MostExpensive:
                {
        ...
        var sql = @$"SELECT Count(A.Title)
            FROM (Select p.Title , i.Price  , i.Id as InventoryId , i.DiscountPercentage , i.Count,
                        p.CategoryId,p.SubCategoryId,p.SecondarySubCategoryId, p.Id as Id , s.SellerStatus
        ...
        var resultSql = @$"SELECT A.Slug,A.Id ,A.Title,A.Price, A.Description, A.BookCount, A.InventoryId,A.DiscountPercentage,A.ImageName
            FROM (Select p.Title , p.Description, i.Price  , i.Id as InventoryId , i.DiscountPercentage,p.ImageName , i.Count as BookCount,
